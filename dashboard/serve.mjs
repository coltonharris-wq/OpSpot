...
function seedAgentActionCatalog(state = {}) {
  ...
  const outbound = generateOutboundQueue(leads, policy);
  const receiptsPath = 'dashboard/state/receipts.jsonl';
  const commonApproval = 'Local research, analysis, ranking, drafting, and queue generation may run alone. External sends/replies/calls, customer-visible changes, Apollo spend, private/financial access, purchases, or destructive actions require explicit Colton approval.';
  const commonInputs = ['target vertical', 'geo/radius', 'suppression list', 'daily caps', 'approved sender policy'];
  return [
    { id: 'aac-google-maps-lead-puller', category: 'source', icon: 'maps', name: 'Google Maps lead puller', status: 'queued', summary: 'Pulls candidate local businesses from Maps/listing sources, then writes raw leads for audit. No outreach.', specialistAgent: 'lead-hunt specialist / research-agent-marley', tools: ['Google Maps/Places source', 'web research', 'local state writer', 'receipt writer'], inputsNeeded: commonInputs, outputs: ['leadSources[] raw candidates', 'source URLs/notes', 'dedupe keys'], outputCount: leads.length, lastRun: 'seeded/local only', nextRun: 'manual or scheduled sourcing window', costCreditRisk: 'low', approvalShort: 'research ok', approvalPolicy: commonApproval, receipts: [receiptsPath, 'lead source URLs per row'], escalationTriggers: ['source blocked/CAPTCHA', 'duplicate-heavy pull', 'non-target vertical drift', 'private data requested'] },
    { id: 'aac-website-gbp-auditor', category: 'audit', icon: 'audit', name: 'Website / GBP auditor', status: 'ready', summary: 'Scores sourced leads for website presence, GBP/reviews, missed-call fit, follow-up fit, and first-touch angle.', specialistAgent: 'website/GBP auditor specialist', tools: ['website fetcher', 'GBP/review research', 'audit scoring rubric', 'receipt writer'], inputsNeeded: ['leadSources[]', 'audit rubric', 'vertical fit rules'], outputs: ['auditScore', 'auditRank', 'reasonWhy', 'angle', 'recommendedFirstTouch'], outputCount: leads.length, lastRun: 'seeded/local only', nextRun: 'after next lead pull', costCreditRisk: 'low', approvalShort: 'audit ok', approvalPolicy: commonApproval, receipts: [receiptsPath, 'audit fields on each leadSource'], escalationTriggers: ['uncertain business identity', 'bad/missing website', 'regulated/unsafe vertical', 'low-confidence score'] },
    { id: 'aac-contact-enricher', category: 'enrich', icon: 'enrich', name: 'Contact enricher', status: 'queued', summary: 'Finds decision-maker hints from cheap/public sources before any paid enrichment gate.', specialistAgent: 'lead-enrich specialist', tools: ['public website/contact page', 'LinkedIn/manual source notes', 'local state writer'], inputsNeeded: ['audit-ranked leads', 'company URL', 'business name/city', 'suppression list'], outputs: ['decisionMakerName', 'title', 'email/phone if public', 'confidence/source'], outputCount: leads.filter(l => l.decisionMakerName || l.decisionMakerEmail || l.decisionMakerPhone).length, lastRun: 'not run', nextRun: 'after audit rank >= threshold', costCreditRisk: 'low', approvalShort: 'public only', approvalPolicy: commonApproval, receipts: [receiptsPath, 'contact source notes'], escalationTriggers: ['private/personal data boundary', 'confidence below threshold', 'contact source conflicts', 'opt-out/suppression hit'] },
    { id: 'aac-apollo-last-gate', category: 'enrich', icon: 'enrich', name: 'Apollo-last enrichment gate', status: 'blocked', summary: 'Only marks Apollo-eligible rows after cheap audit ranking. It never spends credits from the dashboard.', specialistAgent: 'lead-enrich specialist + parent approval gate', tools: ['Apollo plugin/API only after approval', 'credit estimator', 'suppression checker', 'receipt writer'], inputsNeeded: ['top audit-ranked leads', 'cheap research complete', 'suppression check', 'human approval'], outputs: ['apolloEligible', 'credit estimate', 'approved enrichment request'], outputCount: leads.filter(l => l.apolloEligible).length, lastRun: 'eligibility marked only', nextRun: 'after Colton approval', costCreditRisk: 'high', approvalShort: 'approval required', approvalPolicy: 'Apollo spend/enrichment is never automatic. Colton must approve the exact batch, credit estimate, and reason before any Apollo call.', receipts: [receiptsPath, 'Apollo eligibility fields'], escalationTriggers: ['credit estimate > approved cap', 'suppression hit', 'unclear owner/contact', 'batch larger than approved'] },
    { id: 'aac-email-queue-generator', category: 'queue', icon: 'email', name: 'Email queue generator', status: 'ready', summary: 'Drafts audit-backed email queue items with schedule slots and guardrails. No live send integration.', specialistAgent: 'outreach-compose specialist', tools: ['template library', 'audit fields', 'local outboundQueue writer', 'receipt writer'], inputsNeeded: ['audit-backed angle', 'contact email or pending enrichment', 'daily cap', 'quiet-hours policy'], outputs: ['outboundQueue email drafts', 'subject/body', 'scheduledFor', 'sendEnabled=false'], outputCount: outbound.counts.email, lastRun: 'generated from local leadSources', nextRun: 'after audit/enrichment refresh', costCreditRisk: 'low', approvalShort: 'draft only', approvalPolicy: commonApproval, receipts: [receiptsPath, 'outboundQueue[] email items'], escalationTriggers: ['send requested', 'missing contact', 'legal/medical/finance vertical', 'angry/opt-out history'] },
    { id: 'aac-imessage-queue-generator', category: 'queue', icon: 'imessage', name: 'iMessage queue generator', status: 'ready', summary: 'Drafts iMessage queue items for Colton’s approval, respecting quiet hours and daily caps.', specialistAgent: 'outreach-compose specialist + iMessage queue specialist', tools: ['template library', 'local outboundQueue writer', 'iMessage plugin only after approval'], inputsNeeded: ['phone/contact status', 'audit angle', 'daily cap', 'quiet-hours policy'], outputs: ['outboundQueue iMessage drafts', 'scheduledFor', 'sendEnabled=false'], outputCount: outbound.counts.iMessage, lastRun: 'generated from local leadSources', nextRun: 'after audit/enrichment refresh', costCreditRisk: 'medium', approvalShort: 'draft only', approvalPolicy: commonApproval, receipts: [receiptsPath, 'outboundQueue[] iMessage items'], escalationTriggers: ['send requested', 'personal-phone boundary', 'quiet-hours conflict', 'opt-out/angry reply'] },
    { id: 'aac-reply-classifier', category: 'inbound', icon: 'reply', name: 'Reply classifier', status: 'queued', summary: 'Classifies replies as interested, question, objection, opt-out, angry, support, or needs-human.', specialistAgent: 'reply-classifier specialist', tools: ['inbound thread reader after authorization', 'classification rubric', 'approval queue writer'], inputsNeeded: ['authorized inbound source', 'thread text', 'known lead/deal context'], outputs: ['reply class', 'recommended next action', 'approval card'], outputCount: 0, lastRun: 'not run', nextRun: 'after first approved sends/replies', costCreditRisk: 'low', approvalShort: 'classify ok', approvalPolicy: commonApproval, receipts: [receiptsPath, 'approval cards'], escalationTriggers: ['angry/legal/opt-out', 'pricing/contract question', 'customer support issue', 'human identity ambiguity'] },
    { id: 'aac-cold-call-ranker', category: 'sales', icon: 'call', name: 'Cold-call ranker', status: 'ready', summary: 'Ranks the warmest calls from audit signal, outbound prep, Apollo gate, and prior touches.', specialistAgent: 'cold-call ranker specialist', tools: ['leadSources', 'outboundQueue', 'coldCallLeads builder', 'receipt writer'], inputsNeeded: ['audit-ranked leads', 'prepared drafts', 'contact status', 'prior touch history'], outputs: ['coldCallLeads[]', 'warmthScore', 'opener', 'why call now'], outputCount: (state.coldCallLeads || []).length, lastRun: 'generated from local state', nextRun: 'after queue/reply update', costCreditRisk: 'low', approvalShort: 'rank ok', approvalPolicy: commonApproval, receipts: [receiptsPath, 'coldCallLeads[]'], escalationTriggers: ['call requested', 'contact missing', 'DNC/opt-out signal', 'low-confidence business fit'] },
    { id: 'aac-content-generator', category: 'content', icon: 'content', name: 'Content Automation v1', status: 'ready', summary: 'Parent orchestrator routes rough ideas to Instagram, YouTube, Twitch/streaming, clip/repurpose, and approval/publishing specialists for local drafts and receipts.', specialistAgent: 'Parent content orchestrator + channel specialists', tools: ['Mission Control content module', 'Tell Me What You Want builder', 'Business OS spec', 'Instagram/YouTube/Twitch plugin placeholders', 'receipts ledger'], inputsNeeded: ['raw idea/reference/demo/clip', 'target channel', 'CTA', 'proof/receipt expectation'], outputs: ['content brief', 'hook bank', 'platform draft packet', 'approval card', 'receipt log'], outputCount: 5, lastRun: '2026-05-08 09:22 ET', nextRun: 'manual after Colton review', costCreditRisk: 'low', approvalShort: 'publish gated', approvalPolicy: 'Draft locally only. External posts, account changes, DMs/comments, customer proof, boosts/spend, and autopost activation require explicit Colton approval.', receipts: [receiptsPath, 'Business-OS/OPSPOT-CONTENT-AUTOMATION-V1-SKELETON-2026-05-08.md'], escalationTriggers: ['public post requested', 'unsupported customer claim', 'platform account connection', 'boost/spend requested'] },
    { id: 'aac-morning-evening-brief', category: 'brief', icon: 'brief', name: 'Morning / evening brief', status: 'queued', summary: 'Summarizes runs, output counts, cost/credit risks, blockers, and next 1–3 decisions for Colton.', specialistAgent: 'briefing specialist / Adriana decision surface', tools: ['local state reader', 'receipts reader', 'memory writer', 'visible artifact linker'], inputsNeeded: ['mission-control.json', 'receipts.jsonl', 'open blockers', 'approval cards'], outputs: ['brief card', 'decision list', 'memory receipt'], outputCount: 0, lastRun: 'not run', nextRun: 'morning/evening cron', costCreditRisk: 'low', approvalShort: 'brief ok', approvalPolicy: commonApproval, receipts: [receiptsPath, 'memory/YYYY-MM-DD.md'], escalationTriggers: ['decision-worthy blocker', 'spend risk', 'send/call approval needed', 'system failure or stale data'] },
  ];
}
...
function ensureState() {
  ...
  if (!fs.existsSync(STATE_FILE)) {
    ...
    const state = {
      ...
      agentActionCatalog: seedAgentActionCatalog({
        autopilotPolicy: seedAutopilotState().policy,
        leadSources: seedAutopilotState().leadSources,
        coldCallLeads: [],
      }),
...
function readMcState() {
  ...
  const nextActionCatalog = seedAgentActionCatalog(state);
  if (JSON.stringify(state.agentActionCatalog || []) !== JSON.stringify(nextActionCatalog)) { state.agentActionCatalog = nextActionCatalog; changed = true; }
