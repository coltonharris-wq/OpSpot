...
function OpsAutopilotScreen() {
  ...
  const leadSources = (state?.leadSources || []).slice().sort((a,b)=>(b.auditScore||0)-(a.auditScore||0));
    ...
    <div style={{ height: '100%', overflow: 'auto', padding: 24, background: 'var(--canvas)' }}>
      <div style={{ maxWidth: 1280, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 12 }}>
          ...
          <OpsMetric label="Prepared queue" value={queue.length} tone="brand"/>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 420px', gap: 16 }}>
          <OpsPanel title="Audit lane output / ranked local leads" icon={<Icons.Target size={15}/>}>
            <div style={{ display: 'grid', gap: 10 }}>
...
function OpsQueuedSend({ item }) { return <div style={{ border: '1px solid var(--border)', borderRadius: 14, padding: 13, background: 'var(--surface-elevated)' }}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 10 }}><div style={{ fontWeight: 900 }}>{item.auditRank ? `#${item.auditRank} · ` : ''}{item.lead}</div><span className="term" style={{ fontSize: 11, color: 'var(--fg-tertiary)' }}>{item.channel}</span></div><div style={{ color: 'var(--fg-secondary)', fontSize: 13, marginTop: 4 }}>{item.auditBackedAngle || item.angle}</div><div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 8 }}><OpsTag label={`audit ${item.auditScore ?? '—'}`}/><OpsTag label={item.contactStatus || 'contact pending'}/><OpsTag label={item.sendEnabled ? 'send enabled' : 'send disabled'}/></div>{item.draft?.subject && <div style={{ color: 'var(--fg-secondary)', fontSize: 12.5, marginTop: 8 }}><b>Subject:</b> {item.draft.subject}</div>}<div style={{ color: 'var(--fg-tertiary)', fontSize: 12, marginTop: 6 }}>Scheduled: {item.scheduledFor} · Status: {item.status}</div><div style={{ color: 'var(--fg-tertiary)', fontSize: 11.5, marginTop: 5 }}>{item.guardrail || 'Prepared only. No live send integration.'}</div></div>; }
